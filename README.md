...
新版本推荐在 Web 面板的「推送控制」里编辑应用内计划任务，不需要手写 crontab。下面的 `cron` 服务仍保留兼容旧部署；如果你启用了面板计划，请不要在 crontab 里配置同一条日报/周报/月报，避免重复发送。
...
如果你只使用 Web 面板里的应用内计划任务，`cron` 服务可以不启动；保持 `bot` 服务以 `listen` 模式运行即可，它会同时处理 Telegram 命令、采样和应用内 scheduler。
...
推送控制页支持直接用「每日 / 每周 / 每月 + 时间」创建应用内计划任务，配置会保存在 `./data/report_schedules.json`。旧 `crontab` 任务仍可保留为兼容方案，但不会被 Web 面板直接编辑。
